...
case $choice in
    1)
        ...
        clear
        echo ${G}"==> Tiến trình: Đang tải về Distro theo đường dẫn URL mà bạn cung cấp..."${W}
        wget $URL -P ~/$folder/ 
        clear
        ...
        bin=$ds_name-x64.sh
        echo ${G}"==> Tiến trình: Đang tạo file khởi động / đăng nhập vào Distro..."${W}
        cat > $bin <<- EOM
        #!/bin/bash
        cd \$(dirname \$0)
...
EOM
        ...
        chmod +x $bin
        echo ${G}"==> Tiến trình: Xóa file cài đặt Distro giải phóng bộ nhớ..."${W}
        rm -rf $folder/*.tar.*
        echo ${G}"==> Hoàn thành! Giờ bạn có thể đăng nhập vào Distro với câu lệnh: 'bash $bin' và tận hưởng!"${W}
        ;;
    2)
